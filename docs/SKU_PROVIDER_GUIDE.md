...
In sovereign clouds, derive the base ARM endpoint from Azure CLI cloud metadata instead of assuming public Azure:

```bash
az cloud show --name "$(az cloud show --query name -o tsv)" --query endpoints.resourceManager -o tsv
```
...
For Azure Government, the same path is used against the Azure Government Resource Manager host returned by `az cloud show`, such as `https://management.usgovcloudapi.net`.
