...
## Current implementation delta (May 2026)

The repo is no longer only a shell-first inventory script. The current implementation also includes:

- a Functions-backed comparison API that emits canonical service identity into each comparison row,
- an App Service-hosted React frontend that renders canonical identity, fallback diagnostics, and generated artifact state,
- generated catalog artifacts under `data/generated/`, including a canonical identity snapshot and a fallback curation report.

Deployment alignment notes:

- The `azd` web deployment package must stage `data/generated/` artifacts into the App Service runtime package because the hosted shell reads those files for overview and diagnostics context.
- The storage account provisioned by Bicep should include the `comparison-details` blob container so large detail payload offload is explicit in infrastructure, not just created lazily at runtime.
- The pre-deployment optimization pass keeps the Function App on Linux Elastic Premium, disables sticky-session affinity on both hosted apps, lowers the web plan to a Basic Linux SKU, and reuses valid web build/package artifacts to reduce repeat deployment cost and time.

Before the AI expansion slice, the implementation plan now covers four concrete areas:

1. Make canonical identity authoritative in comparison row construction and storage.
2. Generate a reusable fallback curation artifact from comparison output.
3. Add hosted app workflows to isolate fallback rows and identity sources.
4. Keep this spec aligned with the hosted/API implementation surface.
...
- `data/generated/canonical_service_identity.snapshot.json` — normalized canonical identity catalog shared across shell, CLI, and hosted UI.
- `data/generated/canonical_identity_gaps.snapshot.json` — generated fallback identity report built from comparison rows for curation follow-up.
...
- Login: `az login` (or service principal via `az login --service-principal ...`). For Azure Government, first run `az cloud set --name AzureUSGovernment`, then reauthenticate.  
- Access to internet for `prices.azure.com` (public unauthenticated API) when using Commercial Cloud pricing enrichment. [3](https://learn.microsoft.com/en-us/rest/api/cost-management/retail-prices/azure-retail-prices)
...
**Endpoint**: `https://prices.azure.com/api/retail/prices` (Azure Retail Prices API, Commercial Cloud only for `armRegionName` usage)
