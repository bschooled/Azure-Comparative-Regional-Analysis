...
REGION_MAPPING_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REGION_MAPPING_PROJECT_ROOT="$(cd "${REGION_MAPPING_SCRIPT_DIR}/.." && pwd)"
REGION_MAPPING_CACHE_DIR="${CACHE_DIR:-${REGION_MAPPING_PROJECT_ROOT}/.cache}"
REGIONS_CACHE="${REGIONS_CACHE:-${REGION_MAPPING_CACHE_DIR}/azure_regions.json}"
REGIONS_CACHE_METADATA="${REGIONS_CACHE_METADATA:-${REGION_MAPPING_CACHE_DIR}/azure_regions.metadata.json}"

region_cache_matches_active_cloud() {
    local active_cloud=""
    local cached_cloud=""

    [[ -f "$REGIONS_CACHE_METADATA" ]] || return 1

    active_cloud="$(get_active_azure_cloud_name 2>/dev/null || echo AzureCloud)"
    cached_cloud="$(jq -r '.cloud // empty' "$REGIONS_CACHE_METADATA" 2>/dev/null || true)"

    [[ -n "$cached_cloud" && "$cached_cloud" == "$active_cloud" ]]
}

write_region_cache_metadata() {
    local active_cloud=""
    active_cloud="$(get_active_azure_cloud_name 2>/dev/null || echo AzureCloud)"
    jq -cn --arg cloud "$active_cloud" '{cloud: $cloud}' > "$REGIONS_CACHE_METADATA" 2>/dev/null || true
}
...
fetch_azure_regions() {
    ...
    if is_cache_valid "$REGIONS_CACHE" 2>/dev/null && [[ -s "$REGIONS_CACHE" ]] && region_cache_matches_active_cloud; then
        return 0
    fi
    ...
    if [[ -n "$subscription_id" ]] && az rest \
        --method get \
        --url "$(build_arm_url "/subscriptions/${subscription_id}/locations?api-version=2022-12-01")" \
        --query 'value' \
        --output json > "$REGIONS_CACHE" 2>> "${LOG_FILE:-/dev/null}"; then
        local region_count=$(jq '. | length' "$REGIONS_CACHE" 2>/dev/null || echo 0)
        write_region_cache_metadata
        log_success "Retrieved $region_count Azure regions from ARM locations API" >&2
    ...
    if az account list-locations --output json > "$REGIONS_CACHE" 2>> "${LOG_FILE:-/dev/null}"; then
        local region_count=$(jq '. | length' "$REGIONS_CACHE" 2>/dev/null || echo 0)
        write_region_cache_metadata
        log_warning "Retrieved $region_count Azure regions from az account list-locations fallback; availabilityZoneMappings may be incomplete" >&2
        increment_api_call 2>/dev/null
        return 0
    else
...
resolve_region() {
    ...
    validate_cloud_region_alignment "$matched_region" || return 1
