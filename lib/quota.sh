...
fetch_subscription_quota() {
    ...
    local endpoint
    endpoint=$(build_arm_url "/subscriptions/${subscription_id}/providers/Microsoft.Compute/locations/${region}/usages")
...
fetch_service_quota() {
    ...
    if [[ "$resource_type" == "microsoft.network"* ]]; then
        local endpoint
        endpoint=$(build_arm_url "/subscriptions/${subscription_id}/providers/Microsoft.Network/locations/${region}/quotas")
