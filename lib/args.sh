...
validate_arguments() {
    ...
    validate_cloud_region_alignment "$SOURCE_REGION" "$TARGET_REGION" || exit 1
...
EXAMPLES:
    ...
    ./inv.sh --all --source-region eastus --target-region westeurope \\
        ...
        az login
        ./inv.sh --all --source-region usgovvirginia --target-region usgovtexas

NOTES:
        - The tool uses the active Azure CLI cloud metadata for ARM endpoints.
        - In sovereign clouds such as AzureUSGovernment, pricing enrichment is skipped because
            Microsoft's Retail Prices API armRegionName support is documented for Commercial Cloud only.
...
display_config() {
    ...
    log_info "  Active Azure Cloud: $(get_active_azure_cloud_name)"
}
