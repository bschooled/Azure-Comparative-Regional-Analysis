...
RETAIL_PRICES_API="${AZURE_RETAIL_PRICES_API:-https://prices.azure.com/api/retail/prices}"
...
fetch_pricing_data() {
    ...
    if ! azure_retail_prices_supported; then
        local cloud_name
        cloud_name=$(get_active_azure_cloud_name)
        log_warning "Skipping pricing enrichment in ${cloud_name}. Microsoft documents armRegionName pricing support for Commercial Cloud only."
