...
echo "[azd hook] postdeploy: deploying hosted application code and final configuration"
...
if [[ -z "$resource_group" ]]; then
  echo "AZURE_RESOURCE_GROUP is not set. Run 'azd env set AZURE_RESOURCE_GROUP <name>' before deployment." >&2
  exit 1
fi
