...
4. **services_compare Regression Tests** - Availability matching hardening for deployable-provider mode
...
# services_compare availability regression guard
bash tests/test_services_compare_regressions.sh
```

To validate the Python enrichment and rich renderer path too:
...
### Curated Catalog And Python Renderer
- ✅ Curated service catalog builds into JSON and SQLite artifacts
- ✅ Provider comparison JSON can be enriched with curated metadata
- ✅ Rich console rendering works when the local `.venv` is available

### services_compare Regression Guard
- ✅ Filtered provider metadata still returns deployable resource types for representative providers
- ✅ Representative services such as Microsoft.App, Microsoft.Web, Microsoft.Storage, Microsoft.Sql, and Microsoft.ContainerService do not regress to `NOT_AVAILABLE`
