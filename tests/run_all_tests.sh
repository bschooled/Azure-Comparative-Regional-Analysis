...
#  4. services_compare Regression Tests (test_services_compare_regressions.sh)
...
# ==============================================================================
# Run services_compare Regression Tests
# ==============================================================================
run_services_compare_regressions() {
    log_header "SERVICES_COMPARE REGRESSION TESTS"

    if [[ ! -f "$TESTS_DIR/test_services_compare_regressions.sh" ]]; then
        log_warning "services_compare regression file not found, skipping"
        return 0
    fi

    local regression_log="${TEST_RESULTS_DIR}/services_compare_regressions.log"

    if bash "$TESTS_DIR/test_services_compare_regressions.sh" > "$regression_log" 2>&1; then
        log_success "services_compare regression tests passed"
        SUITES_PASSED=$((SUITES_PASSED + 1))
        grep '\[PASS\]' "$regression_log" | head -10 || true
        echo ""
    else
        log_error "services_compare regression tests failed"
        SUITES_FAILED=$((SUITES_FAILED + 1))
        tail -20 "$regression_log" || true
        echo ""
    fi
}
...
4. services_compare Regression Tests
    - Provider metadata helper returns deployable resource types
    - Representative services do not regress to NOT_AVAILABLE
    - Core services remain in expected availability classes
...
Test Logs:
  ...
  - ${TEST_RESULTS_DIR}/quick_validation.log
    - ${TEST_RESULTS_DIR}/services_compare_regressions.log
...
4. Run services_compare regression tests only: bash tests/test_services_compare_regressions.sh
5. Run with actual subscription: inv.sh --subscriptions <subid> --source-region <src> --target-region <tgt>
