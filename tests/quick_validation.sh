...
source "${PROJECT_ROOT}/lib/python_cli.sh"
...
echo ""
echo "[8/10] Fetching Microsoft.Sql SKUs..."
...
echo ""
echo "[9/10] Building curated feature catalog artifacts..."
if ensure_feature_catalog_artifacts; then
    echo "✓ SUCCESS: Curated catalog artifacts are ready"
else
    echo "✗ FAILED: Could not build curated catalog artifacts"
fi

echo ""
echo "[10/10] Rendering sample comparison report if sample JSON exists..."
sample_json="${PROJECT_ROOT}/output/westus2_vs_swedencentral_providers.json"
if [[ -f "$sample_json" ]] && render_provider_comparison_report "$sample_json" >/dev/null; then
    echo "✓ SUCCESS: Python renderer produced a report"
elif [[ -f "$sample_json" ]]; then
    echo "✗ FAILED: Python renderer could not render sample report"
else
    echo "✓ SKIPPED: Sample comparison JSON not present"
fi
